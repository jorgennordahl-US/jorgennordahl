  ...
  return (
    <div className="max-w-3xl mx-auto px-8 py-16">
      ...
      <div className="grid grid-cols-1 gap-4">
        {experiments.map(exp => {
          const href = exp.url || `/experiments/${exp.slug}`
          const isExternal = exp.url?.startsWith('/')
          return (
            <Link
              key={exp.slug}
              href={href}
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-3">
                  {exp.url && (
                    <span className="font-mono text-[10px] bg-teal-50 text-teal-600 px-2 py-0.5 rounded">
                      live demo →
                    </span>
                  )}
